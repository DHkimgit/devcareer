...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($problem_number) || empty($title)) {
        $error_message = "문제 ID와 제목은 필수 항목입니다.";
    } else {
        try {
            $stmt = $pdo->prepare("INSERT INTO programmers_problem (problem_number, title, user_id, summary, solution, level, site_url) VALUES (:problem_number, :title, :user_id, :summary, :solution, :level, :site_url)");
            $stmt->bindParam(':problem_number', $problem_number, PDO::PARAM_STR);
            ...
            $success_message = "프로그래머스 문제가 성공적으로 등록되었습니다.";
            header("Location: /algorithms/index.php?status=programmers_success");
            exit;

        } catch (PDOException $e) {
             if ($e->getCode() == 23000) {
                error_log("Programmers Problem Creation DB Error (Unique Constraint): " . $e->getMessage());
                $error_message = "이미 등록된 문제 ID입니다. 다른 ID를 입력해주세요.";
            } else {
                error_log("Programmers Problem Creation DB Error: " . $e->getMessage());
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="main-content">
            <h1 class="text-2xl font-bold text-gray-800 mb-6">새로운 Programmers 문제 등록</h1>
            ...
            <form action="create_programmers_problem.php" method="POST" class="bg-white p-6 rounded-lg shadow-md">
                <div class="mb-4">
                    <label for="problem_number" class="form-label">문제 ID <span class="text-red-500">*</span></label>
                    <input type="text" id="problem_number" name="problem_number" class="form-input" required placeholder="예: 12345 또는 kit_hash_01">
                </div>
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex items-center space-x-4">
                        <?php for ($i = 0; $i <= 5; $i++): ?>
                            <div class="flex items-center">
                                <input type="radio" id="level_<?php echo $i; ?>" name="level" value="Lv. <?php echo $i; ?>" class="form-radio h-4 w-4 text-primary border-gray-300 focus:ring-primary" <?php if ($i === 0) echo 'checked'; // 기본값으로 Lv.0 선택 ?>>
                                <label for="level_<?php echo $i; ?>" class="ml-2 text-sm text-gray-700">Lv. <?php echo $i; ?></label>
                            </div>
                        <?php endfor; ?>
                    </div>
                </div>
                <div class="mb-4">
                    ...
                    <input type="url" id="site_url" name="site_url" class="form-input" placeholder="https://school.programmers.co.kr/learn/courses/xxxx/lessons/xxxx">
                </div>
