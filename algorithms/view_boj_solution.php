        ...
        try {
            ...
            if (!$problem) {
                ...
                $stmt_categories = $pdo->prepare("
                    SELECT ac.name 
                    FROM algorithm_category ac
                    JOIN boj_problem_category bpc ON ac.id = bpc.category_id
                    WHERE bpc.boj_problem_id = :problem_id
                    ORDER BY ac.name ASC
                ");
                $stmt_categories->bindParam(':problem_id', $problem['id'], PDO::PARAM_INT);
                $stmt_categories->execute();
                $categories = $stmt_categories->fetchAll(PDO::FETCH_COLUMN);
            }
        } catch (PDOException $e) {
            error_log("BOJ Problem View DB Error: " . $e->getMessage());
...
<head>
    ...
    <style>
        ...
        .category-badge { display: inline-block; background-color: #e0e7ff; color: #4338ca; padding: 0.25rem 0.75rem; border-radius: 9999px; font-size: 0.8rem; margin-right: 0.5rem; margin-bottom: 0.5rem;}
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="main-content">
            ...
            <?php elseif ($problem): ?>
                <h1 class="text-3xl font-bold text-gray-800 mb-2"><?php echo htmlspecialchars($problem['title']); ?></h1>
                <p class="text-gray-600 mb-6">BOJ 문제 번호: <?php echo htmlspecialchars($problem['problem_number']); ?></p>
                ...
                <div class="bg-white p-6 rounded-lg shadow-md">
                    ...
                    <div class="problem-section">
                        ...
                        <div>
                            <?php foreach ($categories as $category_name): ?>
                                <span class="category-badge"><?php echo htmlspecialchars($category_name); ?></span>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="mt-8 flex justify-between items-center">
                        ...
                        <a href="/algorithms/edit_boj_problem.php?id=<?php echo $problem['id']; ?>" class="bg-primary text-white px-4 py-2 rounded hover:bg-primary-hover text-sm">수정하기</a>
                    </div>
                </div>
            <?php else: ?>
