...
if ($post_id > 0) {
    try {
        $pdo = getPDO();
        $sql = "SELECT bp.*, p.name AS project_name 
                FROM blog_post bp
                LEFT JOIN projects p ON bp.project_id = p.id
                WHERE bp.id = :post_id AND bp.user_id = :user_id";
        ...
        if (!$post) {
            $error_message = "게시글을 찾을 수 없거나 접근 권한이 없습니다.";
        }
    } catch (PDOException $e) {
        $error_message = "데이터베이스 오류: " . $e->getMessage();
    }
} else {
    $error_message = "잘못된 게시글 ID입니다.";
}

$page_title = $post ? htmlspecialchars($post['title']) : '게시글 보기';
?>
<!DOCTYPE html>
<html lang="ko">
<head>
    ...
    <title><?php echo $page_title; ?> - 블로그</title>
        ...
        <div class="main-content">
            ...
            <?php elseif ($post): ?>
                <div class="post-card">
                    ...
                    <div class="post-meta">
                        <span>작성일: <?php echo date('Y년 m월 d일', strtotime($post['created_at'])); ?></span>
                        <?php if ($post['project_name']): ?>
                            <span>프로젝트: <?php echo htmlspecialchars($post['project_name']); ?></span>
                        <?php endif; ?>
                    </div>
            ...
            <?php else: ?>
                 <div class="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded relative mb-4" role="alert">
                    <span class="block sm:inline">게시글 정보를 불러오는 중입니다. 또는 게시글이 없습니다.</span>
                </div>
            <?php endif; ?>
            <a href="/blog/index.php" class="btn-back">목록으로 돌아가기</a>
        </div>
    </div>
</body>
</html>