...
try {
    $pdo_projects = getPDO();
    $sql_projects = "SELECT id, name FROM projects WHERE user_id = :user_id ORDER BY name ASC";
    if ($stmt_projects = $pdo_projects->prepare($sql_projects)) {
    ...
    $project_fetch_error = "프로젝트 목록을 불러오는 중 오류가 발생했습니다.";
}


$project_id = $title = $content = $tags = "";
$project_id_err = $title_err = $content_err = "";
$success_message = "";

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty(trim($_POST["project_id"]))) {
        $project_id_err = "프로젝트를 선택해주세요.";
    } else {
        ...
        if (!$is_valid_project) {
            $project_id_err = "유효하지 않은 프로젝트입니다.";
            $project_id = "";
        }
    }
    ...
    if (empty($project_id_err) && empty($title_err) && empty($content_err)) {
        try {
            $pdo = getPDO();
            $sql = "INSERT INTO blog_post (user_id, project_id, title, content, tags, created_at, updated_at) VALUES (:user_id, :project_id, :title, :content, :tags, NOW(), NOW())";
    ...
    <div class="content-wrapper">
        ...
        <div class="main-content">
            <div class="bg-white p-8 rounded-lg shadow-md max-w-2xl mx-auto">
                <h1 class="text-2xl font-bold text-primary mb-6">새 프로젝트 게시글 작성</h1>
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                    <div class="mb-4">
                        <label for="project_id" class="block text-sm font-medium text-gray-700 mb-1">프로젝트 선택 <span class="text-red-500">*</span></label>
                        <select name="project_id" id="project_id" class="form-input <?php echo (!empty($project_id_err)) ? 'border-red-500' : ''; ?>">
                            <option value="">-- 프로젝트 선택 --</option>
                            <?php foreach ($user_projects as $project_item): ?>
                                <option value="<?php echo htmlspecialchars($project_item['id']); ?>" <?php echo ($project_id == $project_item['id']) ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($project_item['name']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <?php if(!empty($project_id_err)): ?>
                            <p class="error-message"><?php echo $project_id_err; ?></p>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="content" id="content" class="form-textarea <?php echo (!empty($content_err)) ? 'border-red-500' : ''; ?>"><?php echo htmlspecialchars($content); ?></textarea>
